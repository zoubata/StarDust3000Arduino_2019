...
uint16_t Wheel(byte WheelPos) {
  ...
  char table[32/2][128];//BIT : R1, G1, B1, R2, G2, B2 
  uint32_t color[64];
  ...
  void test()
  {
   ...
   volatile uint32_t * reg= &(PORT->Group[pinPort].OUTTGL.reg);
...
for(int i=0;i<128;i++)
  ...
  *reg= color[(previouscolor^c)/*&0x63*/];
        previouscolor=c;
     ...
     *reg= pinMsk ;//CLK TGL :L
     *reg=  pinMsk ;//CLK TGL :H
